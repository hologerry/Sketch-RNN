...
def kl_divergence(q_mean, q_logvar, p_mean=None, p_logvar=None):
    ...
    kl = p_logvar - q_logvar + (q_logvar.exp() + (q_mean - p_mean) ** 2) / p_logvar.exp() - 1
    kl = 0.5 * kl
    return kl.mean()


class KLLoss(nn.Module):
    def __init__(self, kl_weight=1.0, eta_min=0.01, R=0.99995, kl_min=0.0):
        super().__init__()
    ...
    def forward(self, q_mean, q_logvar, p_mean=None, p_logvar=None):
        if self.kl_weight == 0:
            return torch.tensor(0.0, device=q_mean.device)
