...
from .objective import DrawingLoss, KLLoss
from .param_layer import ParameterLayer
from .rnn import LSTMLayer, _cell_types, init_orthogonal_
...
class SketchRNN(nn.Module):
    ...
    def reset_parameters(self):
        reset = lambda m: (hasattr(m, "reset_parameters") and not isinstance(m, torch.jit.ScriptModule))
        for m in filter(reset, self.children()):
            m.reset_parameters()
        nn.init.normal_(self.init.weight, 0.0, 0.001)
